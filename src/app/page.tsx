        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-gradient-to-r from-sky-50 via-slate-50 to-emerald-50 rounded-3xl border border-[#BAE6FD]/60 shadow-xs">
            <div className="flex items-center gap-4">
              <Avatar name={myWorker.name} size="lg" status={myWorker.status === 'online' ? 'safe' : myWorker.status === 'critical' ? 'danger' : 'warning'} />
              <div>
                ...
                <div key={worker.id} className="p-4 rounded-2xl bg-[#F8FAFC] border border-[#E3EAF5] flex flex-col justify-between space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <Avatar name={worker.name} size="sm" status={worker.status === 'online' ? 'safe' : worker.status} />
                      <div>
            ...
            {workers.slice(0, 4).map((worker) => (
              <Card key={worker.id} variant="interactive" padding="md">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <Avatar name={worker.name} size="md" status={worker.status === 'online' ? 'safe' : worker.status} />
                    <div>
